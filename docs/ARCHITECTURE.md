...
flowchart TD
  ...
  Generator --> Live[LiveProvider\npublic BotHub / DeepSeek]
  ...
  Live -. key only .-> Proxy[Yandex Cloud Function]
  Proxy -. HTTPS .-> Provider[BotHub API]
  Local[Local optional proxy] -. HTTPS .-> OR[OpenRouter API]
```
...
- `server.js` — optional local Node proxy. Reads `OPENROUTER_API_KEY` and `OPENROUTER_MODEL` from ignored `.env`; validates `{subject,body,tone,tags}` and never returns the key.
...
`MessageGenerator` получает decision + minimal employee/request/history/memory context. Публичный `LiveProvider` отвечает за естественный русский текст через Yandex → BotHub/DeepSeek; локальный OpenRouterProvider остаётся альтернативой для разработки. `DemoProvider` сохраняет offline-ready flow при missing key, network/API error, timeout или invalid JSON.
...
Offline deployment — открыть `index.html` или использовать static hosting. Публичная Live demo — GitHub Pages → Yandex API Gateway → Yandex Function → BotHub. Локальная разработка — `node server.js` + ignored `.env`. Следующий production-слой: API/DB вместо localStorage, authentication/RBAC, server-side audit trail, real delivery adapters, consent/opt-out и observability.
